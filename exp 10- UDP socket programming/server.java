...
//socket to receive client data
byte[] buf = new byte[1024];
byte[] cbuf = new byte[1];
DatagramPacket dp = new DatagramPacket(buf,1024);

//packet receiving string from client

ds.receive(dp);
String str = new String(dp.getData(),0,dp.getLength());
System.out.print("String received: "+str+"\n");
DatagramPacket cp = new DatagramPacket(cbuf,1);

//packet receiving character from client
...
DatagramSocket result = new DatagramSocket();
...
//convert int to string for transfer

System.out.print("Count found. Sending data..\n");
DatagramPacket res = new

DatagramPacket(val.getBytes(),val.length(),ip,4000); //send data
to client
